...
public:
	...
	//设置参数
	virtual bool  SetServiceParameter(WORD wServicePort, WORD wMaxConnect,const TCHAR*);
...
public:
	...
	//批量发送
	virtual bool  SendDataBatch(WORD wMainCmdID, WORD wSubCmdID, void * pData, WORD wDataSize,LONG);
	//关闭连接
	virtual bool  CloseSocket(DWORD dwSocketID);
	//设置关闭
	virtual bool  ShutDownSocket(DWORD dwSocketID);
	//允许群发
	virtual bool  AllowBatchSend(DWORD dwSocketID, bool bAllow,LONG);

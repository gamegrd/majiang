...
VOID CInitParameter::LoadInitParameter()
{
	...
	_sntprintf(szInFile,sizeof(szInFile),TEXT("%s\\ServerParameter.ini"),szPath);
	...
	ZeroMemory(tcUrlAdders,sizeof(tcUrlAdders));
	 ...
	 _sntprintf(szDownLoadUrls,CountArray(szDownLoadUrls),TEXT("%s/DownLoad.aspx?KindID=%ld"),tcUrlAdder,1);

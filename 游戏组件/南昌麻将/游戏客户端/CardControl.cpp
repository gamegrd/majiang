...
bool CCardListImage::LoadResource(LPCTSTR uResourceID, int nViewWidth, int nViewHeight)
{
	...
	m_CardListImage.LoadImage(AfxGetInstanceHandle(),uResourceID);
...
bool CCardResource::LoadResource()
{
	...
	m_ImageUserBottom.LoadResource(TEXT("CARD_USER_BOTTOM"),CARD_WIDTH,CARD_HEIGHT);
	...
	//桌子扑克
	m_ImageTableTop.LoadResource(TEXT("CARD_TABLE_TOP"),31,64);
	m_ImageTableLeft.LoadResource(TEXT("CARD_TABLE_LEFT"),53,35);
	m_ImageTableRight.LoadResource(TEXT("CARD_TABLE_RIGHT"),53,35);
	m_ImageTableBottom.LoadResource(TEXT("CARD_TABLE_BOTTOM"),34,70);

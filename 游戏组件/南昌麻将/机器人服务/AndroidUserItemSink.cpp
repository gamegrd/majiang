...
bool  CAndroidUserItemSink::OnEventGameMessage(WORD wSubCmdID, void * pData, WORD wDataSize)
{
	...
	case SUB_S_SEND_CARD_CSGANG:		//发牌消息
		{
			return true;
		}
...
//操作牌
void CAndroidUserItemSink::OnOperateCard( DWORD cbOperateCode,BYTE cbOperateCard )
{
	//删除时间
	m_pIAndroidUserItem->KillGameTimer(IDI_OPERATE_CARD);
...
bool CAndroidUserItemSink::SearchOutCard( tagOutCardResult &OutCardResult )
{
	...
	//计算各种操作得分
	DWORD cbOperateCode[] = { WIK_GANG,WIK_BU_ZHANG,WIK_PENG,WIK_LEFT,WIK_CENTER,WIK_RIGHT,WIK_NULL };
	int nOperateScore[] = { 0,0,0,0,0,0,0 };
	//计算原始分
	CAndroidAI AndroidAi;

	if (hasRule(GAME_TYPE_ZZ_HONGZHONG))
	{
		BYTE cbMagicCardData[1]={m_GameLogic.SwitchToCardIndex(0x35)};
		AndroidAi.SetEnjoinOutCard( cbMagicCardData,1 );
	}
	else
	{
	...
	//判断补
	if(m_cbActionMask & WIK_BU_ZHANG)
	{
		if( m_wCurrentUser == wMeChairId )
		{
			tagGangCardResult GangCardResult;
			...
			ASSERT(nMaxGangScore!=-1&&cbGangCard!=0);
			cbActionCard = cbGangCard;
			nOperateScore[1] = nMaxGangScore-nOrgScore;
		}
		else
		{
			ASSERT( m_cbCardIndex[wMeChairId][m_GameLogic.SwitchToCardIndex(cbActionCard)] > 0 );
	...
	if( m_wCurrentUser == wMeChairId )
	{
		...
		AndroidAi.RemoveCardData( byBadlyIndex );
		AndroidAi.Think();
		nOperateScore[6] = AndroidAi.GetMaxScore()-nOrgScore;
...
bool CAndroidUserItemSink::hasRule(BYTE cbRule)
{

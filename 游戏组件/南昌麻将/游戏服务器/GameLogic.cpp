...
BYTE CGameLogic::GetWeaveCard(DWORD cbWeaveKind, BYTE cbCenterCard, BYTE cbCardBuffer[4])
{
	...
	case WIK_GANG:		//杠牌操作
	case WIK_CS_GANG:	//杠牌操作
	case WIK_BU_ZHANG:	//杠牌操作
		{
			//设置变量
			cbCardBuffer[0]=cbCenterCard;
			cbCardBuffer[1]=cbCenterCard;
...
BYTE CGameLogic::GetUserActionRank(DWORD cbUserAction)
{
	...
	//杠牌等级
	if (cbUserAction&WIK_CS_GANG) { return 3; }
	//杠牌等级
	if (cbUserAction&WIK_BU_ZHANG) { return 3; }
...
WORD CGameLogic::GetChiHuActionRank_CS(const CChiHuRight & ChiHuRight)
{
	...
	if( !(ChiHuRight&CHR_PENGPENG_HU).IsEmpty() )
		wFanShu += 6;
	if( !(ChiHuRight&CHR_JIANGJIANG_HU).IsEmpty() )
		wFanShu += 6;
	if( !(ChiHuRight&CHR_QING_YI_SE).IsEmpty() )
		wFanShu += 6;
	if( !(ChiHuRight&CHR_HAI_DI_LAO).IsEmpty() )
		wFanShu += 6;
	if( !(ChiHuRight&CHR_HAI_DI_PAO).IsEmpty() )
		wFanShu += 6;
	if( !(ChiHuRight&CHR_QI_XIAO_DUI).IsEmpty() )
		wFanShu += 6;
	if( !(ChiHuRight&CHR_HAOHUA_QI_XIAO_DUI).IsEmpty() )
		wFanShu += 12;
	if( !(ChiHuRight&CHR_SHUANG_HAOHUA_QI_XIAO_DUI).IsEmpty() )
		wFanShu += 18;
	if( !(ChiHuRight&CHR_GANG_KAI).IsEmpty() )
		wFanShu += 6;
	if( !(ChiHuRight&CHR_QIANG_GANG_HU).IsEmpty() )
	...
	if( !(ChiHuRight&CHR_QUAN_QIU_REN).IsEmpty() )
		wFanShu += 6;
	if( !(ChiHuRight&CHR_GANG_SHUANG_KAI).IsEmpty() )
		wFanShu *= 2;
	...
	if( !(ChiHuRight&CHR_XIAO_DA_SI_XI).IsEmpty() )
		wFanShu += 2;
	if( !(ChiHuRight&CHR_XIAO_BAN_BAN_HU).IsEmpty() )
		wFanShu += 2;
	if( !(ChiHuRight&CHR_XIAO_LIU_LIU_SHUN).IsEmpty() )
		wFanShu += 2;
	if( !(ChiHuRight&CHR_XIAO_QUE_YI_SE).IsEmpty() )
		wFanShu += 2;
...
DWORD CGameLogic::EstimateEatCard(const BYTE cbCardIndex[MAX_INDEX], BYTE cbCurrentCard)
{
	...
	//吃牌判断
	DWORD cbEatKind=0,cbFirstIndex=0;
...
//杠牌分析
DWORD CGameLogic::AnalyseGangCard(const BYTE cbCardIndex[MAX_INDEX], const tagWeaveItem WeaveItem[], BYTE cbWeaveCount, tagGangCardResult & GangCardResult,BYTE cbGameType)
{

...
public:
	...
	CPoint							m_ptUserImageArray[MAX_CHAIR];		//形象位置
...
public:
	...
	//辅助变量
public:
	INT								m_nUserImageIndex[MAX_CHAIR];		//形象索引
...
public:
	...
	CPngImage						m_ImageUserImage;					//桌面形象
...
protected:
	...
	//坐标加形象
	bool ReadPointEx(POINT & ValuePoint,LPCTSTR pszItem,LPCTSTR pszSubItem,LPCTSTR pszIniFile,INT & nUserImageIndex);
	//读取颜色
...
public:
	...
	//绘画3D头像
	VOID DrawUser3DAvatar(CDC * pDC,INT nXPos,INT nYPos,IClientUserItem * pIClientUserItem,CTableResource * pTableResource);
};
...
public:
	...
	//比赛变量
public:
	DWORD							m_dwMatchTotal;						//报名人数
	DWORD							m_dwWaitting;						//等待人数
	DWORD							m_dwTotalUser;						//参赛人数
	DWORD							m_dwRemainUser;						//剩余人数

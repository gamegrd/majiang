...
CTableResource::CTableResource()
{
	...
	for (WORD i=0;i<MAX_CHAIR;i++)
	{
		...
		m_ptUserImageArray[i].SetPoint(0,0);
		m_rcNameArray[i].SetRect(0,0,0,0);
		m_rcChairArray[i].SetRect(0,0,0,0);
	}

	//辅助变量
	ZeroMemory(m_nUserImageIndex,sizeof(m_nUserImageIndex));
...
VOID CTableResource::DestroyResource()
{
	...
	ZeroMemory(m_ptUserImageArray,sizeof(m_ptUserImageArray));
	...
	if (m_bShowTable==true)
	{
		for (WORD i=0;i<wChairCount;i++)
		{
			...
			if (m_bShowUser==true)
			{
				...
				ReadPointEx(ImagePoint,TEXT("Position"),szSubItem,szParameterFile,m_nUserImageIndex[i]);

				m_ptUserImageArray[i].x=ImagePoint.x;
				m_ptUserImageArray[i].y=ImagePoint.y;
			}
		}
	}
...
bool CTableResource::UpdateResource(CSkinRenderManager * pSkinRenderManager)
{
	...
	//头像
	{
		...
		if(m_ImageUserImage.IsNull()==true)
			m_ImageUserImage.LoadImage(GetModuleHandle(AVATAR_CONTROL_DLL_NAME),TEXT("USER_TABLE_IMAGE"));
...
bool CTableResource::ReadPointEx(POINT & ValuePoint,LPCTSTR pszItem,LPCTSTR pszSubItem,LPCTSTR pszIniFile,INT & nUserImageIndex)
{
	//设置变量
	TCHAR szReadData[64]=TEXT("");
	ZeroMemory(&ValuePoint,sizeof(ValuePoint));
	...
	if (szReadData[0]!=0)
	{
		...
		ValuePoint.x=ReadStringValue(pszString);
		ValuePoint.y=ReadStringValue(pszString);
		nUserImageIndex=ReadStringValue(pszString);
		return true;
	}
		...
		if (pTableResource->m_bShowUser==true)
		{
			...
			if ((bAvertCheatMode==false)&&(m_TableAttribute.pIClientUserItem[i]!=NULL))
			{
				...
				WORD wChairID=pIClientUserItem->GetChairID();
					if(wChairID<CountArray(pTableResource->m_ptUserImageArray)) 
					DrawUser3DAvatar(pDC,pTableResource->m_ptUserImageArray[wChairID].x,pTableResource->m_ptUserImageArray[wChairID].y,pIClientUserItem,pTableResource);
...
//绘画3D形象
VOID CTableView::DrawUser3DAvatar(CDC * pDC,INT nXPos,INT nYPos,IClientUserItem * pIClientUserItem,CTableResource * pTableResource)
{
	//设置变量
	WORD wChairID=pIClientUserItem->GetChairID();

	if(wChairID>CountArray(pTableResource->m_nUserImageIndex)) return ;
	INT nXPosIndex=pTableResource->m_nUserImageIndex[wChairID];
	INT nYPosIndex=pIClientUserItem->GetUserID()%13;//pIClientUserItem->Get3DImageID();
	

	//变量定义
	BOOL bPlaying=(pIClientUserItem->GetUserStatus()==US_PLAYING);
	INT  nPlayingWidth=bPlaying*IMAGE_CX*4;								//游戏状态增量
	INT  nGenderWidth=pIClientUserItem->GetGender()*IMAGE_CX*8;			//性别增量
	...
	if(CD2DEngine::GetD2DEngine())
	{
		//绘画头像
		if (bPlaying==false)
			pTableResource->m_ImageUserImage.DrawImage(pDC,nXPos,nYPos,IMAGE_CX,IMAGE_CY,
								nXPosIndex*IMAGE_CX+nGenderWidth,nYPosIndex*IMAGE_CY);
		else
			pTableResource->m_ImageUserImage.DrawImage(pDC,nXPos,nYPos,IMAGE_CX,IMAGE_CY,
								nXPosIndex*IMAGE_CX+nGenderWidth+nPlayingWidth,nYPosIndex*IMAGE_CY);
	}
	else
	{
		IImageItemControl * pIImageItemControl=CImageItemControl::GetInstance();
		if(pIImageItemControl==NULL) 
		{
			AfxMessageBox(TEXT("CImageItemControl is NULL"));
			return ;
		}
		if (bPlaying==false)
		{
			pIImageItemControl->DrawImage(pDC,nXPos,nYPos,nXPosIndex*IMAGE_CX+nGenderWidth,nYPosIndex*IMAGE_CY);
		}
		else
		{
			pIImageItemControl->DrawImage(pDC,nXPos,nYPos,nXPosIndex*IMAGE_CX+nGenderWidth+nPlayingWidth,nYPosIndex*IMAGE_CY);
		}
	}
	return ;
}
...
bool CTableViewFrame::ConfigTableFrame(WORD wTableCount, WORD wChairCount, DWORD dwServerRule, WORD wServerType, WORD wServerID, LPCTSTR pszResDirectory)
{
	...
	if((m_wServerType&GAME_GENRE_MATCH)!=0)
	{
		...
		_sntprintf(szBillUrl,CountArray(szBillUrl),TEXT("%s/Ads/MatchInfo.asp?ServerID=%d"),szPlatformLink,wServerID);
...
bool CTableViewFrame::FlashGameChair(WORD wTableID, WORD wChairID)
{
	...
	//错误判断
	if (pITableView==NULL||TRUE)//删掉椅子lw
	{
		ASSERT(FALSE);
		return false;
	}
...
VOID CTableViewFrame::OnPaint()
{
	...
	if((m_wServerType&GAME_GENRE_MATCH)!=0)
	{
		...
		if(!m_ImageTableTL.IsNull() && !m_ImageTableTM.IsNull() && !m_ImageTableTR.IsNull())
		{
			...
			m_ImageTableTL.DrawImageTile(pBufferDC, rcRect);
			...
			m_ImageTableTR.DrawImageTile(pBufferDC, rcRect);
		
			//创建字体
			CFont DrawFont;
			DrawFont.CreateFont(18,0,0,0,300,0,0,0,134,3,2,1,2,TEXT("黑体"));
			...
			DrawNumberString(pBufferDC, m_ImageNumberYellow, m_dwWaitting, nXPos,nYPos+15,TA_LEFT);
			DrawNumberString(pBufferDC, m_ImageNumberGreen, m_dwTotalUser, nXPos+38,nYPos+55,TA_RIGHT);	
			m_ImageLine.DrawImage(pBufferDC,nXPos+22,nYPos+10);
		...
		if ((m_ImageEnterMatch.IsNull()==false) && m_ImageLeaveMatch.IsNull()==false)
		{
			...
			if(m_cbMatchStatus==MS_NULL)
			{
				...
				m_ImageEnterMatch.BitBlt(pBufferDC->GetSafeHdc(),rcClient.Width()-ENTER_MATCH_BUTTON_X,ENTER_MATCH_BUTTON_Y,
			...
			else if(m_cbMatchStatus==MS_SIGNUP)
			{
				...
				m_ImageLeaveMatch.BitBlt(pBufferDC->GetSafeHdc(),rcClient.Width()-ENTER_MATCH_BUTTON_X,ENTER_MATCH_BUTTON_Y,

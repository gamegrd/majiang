...
VOID CPlatformFrame::RectifyControl(INT nWidth, INT nHeight)
{
	...
	//其他控件
	DeferWindowPos(hDwp,m_PlatformPublicize,NULL,320,7,271,65,uFlags);
...
CServerViewItem * CPlatformFrame::EntranceServerItem(CGameServerItem * pGameServerItem)
{
	...
	try
	{
		...
		//配置房间
		m_pServerViewItem->InitServerViewItem(pGameServerItem,m_wAVServerPort,m_dwAVServerAddr);
	}
...
BOOL CPlatformFrame::OnEraseBkgnd(CDC * pDC)
{
	...
	if (m_bHideGameList==false)
	{
		...
		_sntprintf(szOnLineInfo,CountArray(szOnLineInfo),TEXT("同时在线人数：%ld"),m_dwAllOnLineCount);
...
INT CPlatformFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_ToolTipCtrl.AddTool(&m_btNavigation[2],TEXT("客户服务"));
	m_ToolTipCtrl.AddTool(&m_btNavigation[3],TEXT("比赛"));
	m_ToolTipCtrl.AddTool(&m_btNavigation[4],TEXT("推广员"));
	...
	_sntprintf(szBillUrl,CountArray(szBillUrl),TEXT("%s/Ads/Platform.aspx"),szPlatformLink);

...
#ifdef _UNICODE
	#define VER_IImageItemControl INTERFACE_VERSION(1,1)
	static const GUID IID_IImageItemControl={0x616f13f6,0xb093,0x4ce4,0x87,0x43,0x69,0x06,0x6b,0x9a,0x90,0x52};
#else
	#define VER_IImageItemControl INTERFACE_VERSION(1,1)
	static const GUID IID_IImageItemControl={0x64ba0bfe,0x9734,0x449f,0xbf,0x0c,0x8f,0x1b,0x4b,0x48,0x77,0xe9};
#endif

//头像组件
interface IImageItemControl : public IUnknownEx
{
	//信息接口
public:
	//头像数目
	virtual WORD GetImageCount()=NULL;
	//获取尺寸
	virtual VOID GetImageItemSize(CSize & SizeFace)=NULL;
...
public:
	//绘画头像
	virtual VOID DrawImage(CDC * pDC, INT nXPos, INT nYPos, INT nSrcWidth, INT nSrcHeight)=NULL;

	//绘画接口
//public:
	//绘画头像
	//virtual VOID DrawImageFree(CDC * pDC, INT nXPos, INT nYPos, INT nWidth, INT nHeight, WORD wFaceID)=NULL;
	//绘画头像
	//virtual VOID DrawImagePlaying(CDC * pDC, INT nXPos, INT nYPos, INT nWidth, INT nHeight, WORD wFaceID)=NULL;
};
...
DECLARE_MODULE_HELPER(ImageItemControl,AVATAR_CONTROL_DLL_NAME,"CreateImageItemControl")
